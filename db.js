        ...
        return pgPool.query('SELECT COUNT(*) FROM lotto_hotspots').then(res => {
          if (parseInt(res.rows[0].count) === 0) {
            return pgPool.query(`
              ...
              ('라이프마트', '인천 남동구 간석동 395-2', '1등 18회 / 2등 55회 이상', '인천 남동구 핵심 상권에 위치하여 서울/수도권을 장악하는 강력한 1등 행운 충전소', '경기'),
              ('로또명당 인주점', '충남 아산시 인주면 서해로 512-2', '1등 17회 / 2등 50회 이상', '충청권 최고의 1등 배출 명가로 아산만 방조제 길목에 위치하여 많은 방문객이 모여드는 성지', '충청'),
          ...
          `, (err2) => {
            ...
            lottoSqlite.get('SELECT COUNT(*) as count FROM lotto_hotspots', [], (err3, row) => {
              ...
              if (row.count === 0) {
                ...
                stmt.run('라이프마트', '인천 남동구 간석동 395-2', '1등 18회 / 2등 55회 이상', '인천 남동구 핵심 상권에 위치하여 서울/수도권을 장악하는 강력한 1등 행운 충전소', '경기');
                stmt.run('로또명당 인주점', '충남 아산시 인주면 서해로 512-2', '1등 17회 / 2등 50회 이상', '충청권 최고의 1등 배출 명가로 아산만 방조제 길목에 위치하여 많은 방문객이 모여드는 성지', '충청');
